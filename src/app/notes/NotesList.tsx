  ...
  return <aside id="logo-sidebar"
    ...
    <div className="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-gray-800  p-2 text-gray-900 rounded-lg dark:text-white">
      <div className="border-2 top-0 left-0">
        <form aria-label={'select note or add new'} className="display: flex align-items: center" onSubmit={addNoteClick}>
          <button aria-label={'select note or add new'} className="hover:bg-gray-100 dark:hover:bg-gray-700 group mt-3 m-2" type="submit">
        ...
        {notes.map(note =>
          <li key={note.id}>
            <div className="flex w-auto">
              <button aria-label="delete note" onClick={e => deleteNote(note.id)}>
