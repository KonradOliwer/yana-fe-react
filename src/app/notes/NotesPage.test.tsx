...
const noteToSave = { id: 'noteToSaveId', name: 'note to save name', content: 'note to save content' };
...
jest.mock('./NotesList', () => {
  return {
    __esModule: true,
    default: ({ notes, createNewNote, deleteNote, currentNoteId }: {
      notes: Note[],
      ...
      return <div>
        ...
        <div>
          <button onClick={() => createNewNote(createNewNoteName, createNewNoteContent)}>createNote</button>
          <button onClick={() => deleteNote(noteToDeleteId)}>deleteNote</button>
        </div>
      </div>;
    }
  };
      ...
      return <div>
        {note ? (
          <>
            ...
            <div data-testid={contentPassedToNotePageTestId}>{note?.content}</div>
          </>
...
test('/notes/:id result in passing note with this id to NotePage', async () => {
  (api.getNotes as jest.Mock).mockResolvedValue([exampleNote, { id: '2', name: 'note2', content: 'content2' }]);
  ...
  expect(screen.getByTestId(namePassedToNotePageTestId)).toHaveTextContent('note2');
  expect(screen.getByTestId(contentPassedToNotePageTestId)).toHaveTextContent('content2');
});
...
test('list of notes passed to NotesList', async () => {
  (api.getNotes as jest.Mock).mockResolvedValue([{ id: '1', name: 'note1', content: 'content1' }, { id: '2', name: 'note2', content: 'content2' }]);
...
test('saveNoteChanges fails on updateNote with note not found and user confirms the intent to create new note', async () => {
  (api.getNotes as jest.Mock).mockResolvedValue([exampleNote]);
  (api.updateNote as jest.Mock).mockRejectedValue({ code: api.NoteApiErrorCode.NOT_FOUND });
  ...
  expect(api.addNote).toHaveBeenCalledWith({ name: noteToSave.name, content: noteToSave.content });
  expect(screen.getAllByRole('listitem')).toHaveLength(2);
  expect(screen.getByText(noteToSave.name)).toBeInTheDocument();
  expect(screen.getByText(noteToSave.content)).toBeInTheDocument();
});
...
test('saveNoteChanges fails on updateNote with note not found and user reject the intent to create new note', async () => {
  ...
  expect(api.addNote).not.toHaveBeenCalledWith({ name: noteToSave.name, content: noteToSave.content });
  expect(screen.getAllByRole('listitem')).toHaveLength(1);
});
...
test('deleteNote passed to NotesList triggers deleteNote and refresh notes list if user confirms the action', async () => {
  ...
  await act(async () => {
    fireEvent.click(screen.getByText('deleteNote'));
  });
...
test('deleteNote when confirmed and currently displayed note is being deleted redirect to notes/ and clear current note', async () => {
  (api.getNotes as jest.Mock).mockResolvedValue([{ id: noteToDeleteId, name: 'note1', content: 'content1' }]);
  (api.deleteNote as jest.Mock).mockResolvedValue(Promise.resolve());
...
test('Submitting note name in NotesList select note of this name for the list', async () => {
  (api.getNotes as jest.Mock).mockResolvedValue([{ id: '1', name: createNewNoteName, content: createNewNoteContent }]);
  ...
  await act(async () => {
    fireEvent.click(screen.getByText('createNote'));
  });
...
test('Submitting note name in NotesList create new note if note with this name does not exist', async () => {
  (api.addNote as jest.Mock).mockResolvedValue({ id: '1', name: createNewNoteName, content: createNewNoteContent });
  (api.getNotes as jest.Mock)
    .mockResolvedValueOnce([exampleNote])
    .mockResolvedValueOnce([exampleNote])
    .mockResolvedValueOnce([exampleNote, { id: '1', name: createNewNoteName, content: createNewNoteContent }]);
  ...
  expect(api.addNote).toHaveBeenCalledWith({ name: createNewNoteName, content: createNewNoteContent });
  expect(history.location.pathname).toBe('/notes/1');
});